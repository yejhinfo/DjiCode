 ...
 * build: g++ camera.cpp -ljpeg -o capture
 */
...
{
  JSAMPARRAY image;
  image = (unsigned char**)calloc(height, sizeof (JSAMPROW));
  for (size_t i = 0; i < height; i++) {
    image[i] = (unsigned char*) calloc(width * 3, sizeof (JSAMPLE));
...
{
  uint8_t* rgb = (unsigned char*)calloc(width * height * 3, sizeof (uint8_t));
  for (size_t i = 0; i < height; i++) {
